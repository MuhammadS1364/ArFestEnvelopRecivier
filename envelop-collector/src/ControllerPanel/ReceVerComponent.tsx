  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const readyToReceive = data.filter((p: Programme) => 
          String(p.IsSummitted).toLowerCase() === "true" && 
          String(p.IsRecivied).toLowerCase() !== "true"
        );
        setProgrammes(readyToReceive);
      } catch (err) {
        console.error(err);
      } finally {
        setIsLoading(false);
  ...
  const handleReceive = async (code: string) => {
    ...
    try {
      await fetch(`${SHEET_API_ENDPOINT}/CODE/${code}?sheet=Non-Stage`, {
        ...
        body: JSON.stringify({ IsRecivied: "TRUE", Receiver: controllerEmail }),
      });
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-gray-200 gap-4">
        ...
        <select 
          ...
          className="w-full sm:w-auto px-4 py-2 rounded-lg border border-gray-300 text-sm outline-none focus:ring-2 focus:ring-green-500"
        >
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        {isLoading ? (
           <div className="p-8 text-center text-gray-500">Loading incoming programmes...</div>
        ) : (
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredData.length === 0 ? (
                  <tr><td colSpan={4} className="px-6 py-8 text-center text-gray-500">No items waiting.</td></tr>
                ) : (
                  filteredData.map((prog) => (
                    <tr key={prog.CODE} className="hover:bg-green-50/50">
                      ...
                      <td className="px-6 py-4 text-right">
                        <button
                          onClick={() => handleReceive(prog.CODE)}
                          disabled={actionLoading === prog.CODE}
                          className="px-4 py-2 bg-green-600 text-white rounded-lg font-medium hover:bg-green-700 disabled:opacity-50"
                        >
                          {actionLoading === prog.CODE ? "Updating..." : "Mark Received"}
                        </button>
                      </td>
                    </tr>
                  ))
